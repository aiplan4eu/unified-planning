...
Expression = Union[upf.model.FNode, 'upf.model.Fluent', 'upf.model.Object', 'upf.model.ActionParameter', 'upf.model.Variable', bool, int, float, Fraction]
BoolExpression = Union[upf.model.FNode, 'upf.model.Fluent', 'upf.model.ActionParameter', bool]
...
class ExpressionManager(object):
    ...
    def __init__(self, env: 'upf.environment.Environment'):
        self.env = env
        self.expressions: Dict[upf.model.FNodeContent,upf.model.FNode] = {}
    ...
    def auto_promote(self, *args: Union[Expression, Iterable[Expression]]) -> List[upf.model.FNode]:
        tuple_args = self._polymorph_args_to_tuple(*args)
        res = []
        for e in tuple_args:
            if isinstance(e, upf.model.Fluent):
                res.append(self.FluentExp(e))
            elif isinstance(e, upf.model.ActionParameter):
                res.append(self.ParameterExp(e))
            elif isinstance(e, upf.model.Variable):
                res.append(self.VariableExp(e))
            elif isinstance(e, upf.model.Object):
    ...
    def create_node(self, node_type: int, args: Iterable[upf.model.FNode],
                    payload: Union['upf.model.Fluent', 'upf.model.Object', 'upf.model.ActionParameter', 'upf.model.Variable', bool, int, Fraction, Tuple['upf.model.Variable', ...]] = None) ->upf.model.FNode:
        content =upf.model.FNodeContent(node_type, args, payload)
        if content in self.expressions:
            return self.expressions[content]
        else:
            n =upf.model.FNode(content, self._next_free_id)
            self._next_free_id += 1
            self.expressions[content] = n
            self.env.type_checker.get_type(n)
            return n

    def And(self, *args: Union[BoolExpression, Iterable[BoolExpression]]) ->upf.model.FNode:
    ...
    def Or(self, *args: Union[BoolExpression, Iterable[BoolExpression]]) ->upf.model.FNode:
    ...
    def Not(self, expression: BoolExpression) ->upf.model.FNode:
    ...
    def Implies(self, left: BoolExpression, right: BoolExpression) ->upf.model.FNode:
        """ Creates an expression of the form:
            left -> right
    ...
    def Iff(self, left: BoolExpression, right: BoolExpression) ->upf.model.FNode:
        """ Creates an expression of the form:
            left <-> right
        Restriction: Left and Right must be of boolean type
        """
    ...
    def Exists(self, expression: BoolExpression, *vars: upf.model.Variable) ->upf.model.FNode:
        """ Creates an expression of the form:
        ...
        for v in vars:
            if not isinstance(v, upf.model.Variable):
    ...
    def Forall(self, expression: BoolExpression, *vars: upf.model.Variable) ->upf.model.FNode:
        """ Creates an expression of the form:
    ...
    def FluentExp(self, fluent: 'upf.model.Fluent', params: Tuple[Expression, ...] = tuple()) ->upf.model.FNode:
    ...
    def ParameterExp(self, param: 'upf.model.ActionParameter') ->upf.model.FNode:
    ...
    def Int(self, value: int) ->upf.model.FNode:
    ...
    def Real(self, value: Fraction) ->upf.model.FNode:
    ...
    def Plus(self, *args: Union[Expression, Iterable[Expression]]) ->upf.model.FNode:
        """ Creates an expression of the form:
            args[0] + ... + args[n]
        """
        tuple_args = tuple(self.auto_promote(*args))
    ...
    def Minus(self, left: Expression, right: Expression) ->upf.model.FNode:
        """ Creates an expression of the form: left - right."""
        left, right = self.auto_promote(left, right)
        return self.create_node(node_type=op.MINUS, args=(left, right))

    def Times(self, *args: Union[Expression, Iterable[Expression]]) ->upf.model.FNode:
        """ Creates an expression of the form:
            args[0] * ... * args[n]
        """
        tuple_args = tuple(self.auto_promote(*args))
    ...
    def Div(self, left: Expression, right: Expression) ->upf.model.FNode:
        """ Creates an expression of the form: left / right."""
        left, right = self.auto_promote(left, right)
        return self.create_node(node_type=op.DIV, args=(left, right))

    def LE(self, left: Expression, right: Expression) ->upf.model.FNode:
        """ Creates an expression of the form: left <= right."""
        left, right = self.auto_promote(left, right)
        return self.create_node(node_type=op.LE, args=(left, right))

    def GE(self, left: Expression, right: Expression) ->upf.model.FNode:
        """ Creates an expression of the form: left >= right."""
        left, right = self.auto_promote(left, right)
        return self.create_node(node_type=op.LE, args=(right, left))

    def LT(self, left: Expression, right: Expression) ->upf.model.FNode:
        """ Creates an expression of the form: left < right."""
        left, right = self.auto_promote(left, right)
        return self.create_node(node_type=op.LT, args=(left, right))

    def GT(self, left: Expression, right: Expression) ->upf.model.FNode:
        """ Creates an expression of the form: left > right."""
        left, right = self.auto_promote(left, right)
        return self.create_node(node_type=op.LT, args=(right, left))

    def Equals(self, left: Expression, right: Expression) ->upf.model.FNode:
        """ Creates an expression of the form: left == right."""
        left, right = self.auto_promote(left, right)
