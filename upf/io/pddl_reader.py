...
class PDDLReader:
    ...
    def _parse_exp(self, problem: upf.model.Problem, act: typing.Optional[upf.model.Action],
                   var: Dict[str, upf.model.Variable], exp: Union[ParseResults, str]) -> upf.model.FNode:
        stack = [(var, exp, False)]
        solved: List[upf.model.FNode] = []
        while len(stack) > 0:
            ...
            if status:
                if exp[0] == '-' and len(exp) == 2: # unary minus
                    ...
                    raise upf.exceptions.UPFUnreachableCodeError
            else:
    ...
    def _add_effect(self, problem: upf.model.Problem,
                    act: Union[upf.model.InstantaneousAction, upf.model.DurativeAction],
                    exp: Union[ParseResults, str],
                    cond: Union[upf.model.FNode, bool] = True,
                    timing: typing.Optional[upf.model.Timing] = None):
        to_add = [(exp, cond)]
        while to_add:
    ...
    def _add_condition(self, problem: upf.model.Problem, act: upf.model.DurativeAction,
                       exp: Union[ParseResults, str],
                       vars: typing.Optional[Dict[str, upf.model.Variable]] = None):
        to_add = [(exp, vars)]
        while to_add:
            ...
            if op == 'and':
                ...
                t_all = upf.model.OpenInterval(upf.model.StartTiming(), upf.model.EndTiming())
                cond = self._parse_exp(problem, act, {} if vars is None else vars, exp[2])
                if vars is not None:
                    cond = self._em.Forall(cond, *vars.values())
    ...
    def _add_timed_effects(self, problem: upf.model.Problem,
                           act: upf.model.DurativeAction,
                           eff: ParseResults):
        to_add = [eff]
        while to_add:
            eff = to_add.pop(0)
    ...
    def parse_problem(self, domain_filename: str,
                      problem_filename: typing.Optional[str] = None) -> 'upf.model.Problem':
        domain_res = self._pp_domain.parseFile(domain_filename)

        problem = upf.model.Problem(domain_res['name'], self._env,
