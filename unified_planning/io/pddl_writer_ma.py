                ...
                if direct_sons:
                    ...
                    for agent in self.problem._agents:
                        ...
                        if type.type in direct_sons:
                            out.write(
                                f'        {" ".join([self._type_name_or_object_freshname(t) for t in direct_sons])} - (either {self._type_name_or_object_freshname(current_type)} agent)\n'
                            )
                            break
                        else:
                            out.write(
        ...
        for f in self.problem.fluents:
            if f.type.is_bool_type():
                    ...
                    if query:
                        ...
                        for p in f.signature:
                            if p.type.is_user_type:
                                if (
                                    self._type_name_or_object_freshname(p)
                                    not in dict[query[0]]
                                ):
                ...
                for p in f.signature():
                    if p.is_user_type():
                        params.append(
                            f" ?p{str(i)} - {self._type_name_or_object_freshname(p)}"
                        )
                        i += 1
                    else:
        ...
        if self.problem.get_flu_functions():
                    ...
                    if query:
                        ...
                        for p in f.signature:
                            if p.type.is_user_type():
                                if (
                                    self._type_name_or_object_freshname(p)
                                    not in functions_dict[user_type_name]
                                ):
                                    type = self._type_name_or_object_freshname(p)
            ...
            for i, k in functions_dict.items():
                if len(k) > 2:
                    i = i.replace("_", "")
                    out.write(
                        f'  ({i} ?{k[0][0]} - {k[0]}) - (either {" ".join([t for t in k[1:]])})\n'
                    )
                else:
                    i = i.replace("_", "")
            ...
            if isinstance(a, up.model.InstantaneousAction):
                ...
                if len(a.preconditions) > 0:
                    ...
                    for ap in a.parameters:
                        if type == ap.type:
                            ...
                            parameter_name = str(
                                self.problem.object(self._ag).type.name
                            )[0]
                    ...
                    for p in a.preconditions:
                        if (
                            len(p._content.args) > 1
                            and p.is_fluent_exp
                            and p.fluent() in self.problem.get_flu_functions()
                        ):
                            flu_name = p.fluent().name
                            flu_name = self.remove_underscore(flu_name)
                            out.write(
                                f' (= ({flu_name} ?{")".join([str(p.arg(0))])}) ?{p.arg(1)})'
                            )
                        else:
                ...
                if len(a.effects) > 0:
                    ...
                    for e in a.effects:
                        if e.is_conditional():
                            if (
                                len(e.fluent._content.args) > 1
                                and e.fluent in self.problem.get_flu_functions()
                            ):
                                flu_name = e.fluent.name
                                # out.write(f'\n :precondition (and {" ".join([converter.convert(p)])})')
                                flu_name = self.remove_underscore(flu_name)
                                out.write(
                                    f' (when (= ({flu_name} ?{")".join([str(e.arg(0))])}) ?{e.arg(1)})'
                                )
                            else:
                        ...
                        if e.value.is_true():
                            if (
                                len(e.fluent._content.args) > 1
                                and e.fluent.fluent()
                                in self.problem.get_flu_functions()
                            ):
                                flu_name = e.fluent.fluent().name
                                flu_name = self.remove_underscore(flu_name)
                                out.write(
                                    f' (assign ({flu_name} ?{")".join([str(e.fluent.arg(0))])}) ?{e.fluent.arg(1)})'
                                )
                            else:
                            ...
                            if (
                                ...
                                flu_name = self.remove_underscore(flu_name)
                                out.write(
                                    f' (not (= ({flu_name} ?{")".join([str(e.fluent.arg(0))])}) ?{e.fluent.arg(1)})'
                                )
                            else:
                            ...
                            if (
                                ...
                                flu_name = self.remove_underscore(flu_name)
                                out.write(
                                    f' (increase (= ({flu_name} ?{")".join([str(e.fluent().arg(0))])}) ?{e.fluent().arg(1)} {converter.convert(e.value())})'
                                )
                            else:
                                new_e = converter.convert(e.fluent())
                                new_e = self.remove_underscore(new_e)
                                out.write(
                                    f" (increase {new_e} {converter.convert(e.value())})"
                                )
                            ...
                            if (
                                ...
                                flu_name = self.remove_underscore(flu_name)
                                out.write(
                                    f' (decrease (= ({flu_name} ?{")".join([str(e.fluent().arg(0))])}) ?{e.fluent().arg(1)} {converter.convert(e.value())})'
                                )
                            else:
                                new_e = converter.convert(e.fluent())
                                new_e = self.remove_underscore(new_e)
                                out.write(
                                    f" (decrease {new_e} {converter.convert(e.value())})"
                                )
                            ...
                            new_e = self.remove_underscore(new_e)
                            out.write(
                                f" (assign {new_e} {converter.convert(e.value())})"
                            )
                        if e.is_conditional():
                            out.write(f")")
                    ...
                    if a in costs:
                        out.write(
                            f" (increase total-cost {converter.convert(a.cost())})"
                        )
                    out.write(")")
                out.write(")\n")
