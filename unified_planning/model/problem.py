...
class Problem:
    ...
    def print_pyhton_commands(self) -> Iterator[str]:
        yield 'import unified_planning'
        yield 'from unified_planning.shortcuts import *'
        types_hierarchy = self.user_types_hierarchy()
        # define user types
        stack: List['unified_planning.model.types.Type'] = types_hierarchy[None]
        while stack:
            current_type = stack.pop()
            if current_type.father() is not None:
                yield f'{current_type.name()} = UserType("{current_type.name()}", "{current_type.father().name()}")'
            else:
                yield f'{current_type.name()} = UserType("{current_type.name()}")'
        # define fluents
        for fluent in self.fluents():
            signature: List[str] = [_print_python_type(t) for t in fluent.signature()]
            yield f'{fluent.name()} = Fluent("{fluent.name()}", {_print_python_type(fluent.type())}, {str(signature)})'
        # define objects
        for object in self.all_objects():
            yield f'{object.name()} = Object("{object.name()}", {_print_python_type(object.type())})'
        #define actions
        for inst_act in self.instantaneous_actions():
            parameters: str = ''
            
            yield f'{inst_act.name()} = '
