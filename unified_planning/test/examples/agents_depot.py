...
def get_example_problems():
    ...
    # Avaiable e lifting sostituiti con Avaiable_ e lifting_
    available_ = Fluent("available_", hoist=hoist)
    lifting_ = Fluent("lifting", hoist=hoist, crate=crate)
    ...
    drop.add_precondition(lifting_(x, y))

    drop.add_effect(available_(x), True)
    drop.add_effect(at(y, p), True)
    drop.add_effect(clear(y), True)
    drop.add_effect(on(y, z), True)
    drop.add_effect(lifting_(x, y), False)
    ...
    load.add_precondition(lifting_(x, y))

    load.add_effect(In(y, z), True)
    load.add_effect(available_(x), True)
    load.add_effect(lifting_(x, y), False)
    ...
    unload.add_precondition(available_(x))
    unload.add_precondition(In(y, z))

    unload.add_effect(lifting_(x, y), True)
    unload.add_effect(In(y, z), False)
    unload.add_effect(available_(x), False)
    ...
    depot0_a.add_fluent(lifting_, default_initial_value=False)
    # depot0_a.add_fluent(available, default_initial_value=False)
    depot0_a.add_fluent(available_, default_initial_value=False)

    distributor0_a.add_fluent(lifting_, default_initial_value=False)
    ...
    distributor1_a.add_fluent(lifting_, default_initial_value=False)
