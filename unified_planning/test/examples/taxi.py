...
def get_example_problems():
    ...
    taxi = UserType("taxi")
    taxi1 = Agent("taxi1", problem)
    taxi2 = Agent("taxi2", problem)
    ...
    directly_connected = Fluent("directly_connected", BoolType(), l1=location, l2=location)
    # Da mettere nell'agente (PRIVATI)
    goal_of = Fluent("goal_of", BoolType(), location=location)
    driving = Fluent("driving", taxi=taxi)


    person1.add_fluent(goal_of, default_initial_value=False)
    person1.add_fluent(pos, default_initial_value=False)
    person1.add_fluent(In, default_initial_value=False)

    person2.add_fluent(goal_of, default_initial_value=False)
    person2.add_fluent(pos, default_initial_value=False)
    person2.add_fluent(In, default_initial_value=False)

    #taxi1.add_fluent(pos, default_initial_value=False)
    taxi1.add_fluent(In, default_initial_value=False)
    taxi1.add_fluent(driving, default_initial_value=False)

    #taxi2.add_fluent(pos, default_initial_value=False)
    taxi2.add_fluent(In, default_initial_value=False)
    taxi2.add_fluent(driving, default_initial_value=False)
    ...
    taxi1.add_action(drive_t)
    taxi2.add_action(drive_t)

    problem.add_agent(person1)
    problem.add_agent(person2)
    problem.add_agent(taxi1)
    problem.add_agent(taxi2)
    ...
    problem.set_initial_value(Dot(taxi1, driving(t1)), True)
    problem.set_initial_value(Dot(taxi2, driving(t2)), True)
    ...
    w.write_ma_domain('taxi')
    w.write_ma_problem('taxi')



    plan = None
    taxi = Example(problem=problem, plan=plan)
    problems["ma_taxi"] = taxi
    return problems


problems = get_example_problems()
problem = problems["ma_taxi"].problem
