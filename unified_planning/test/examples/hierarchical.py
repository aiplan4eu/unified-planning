...
def get_example_problems():
    ...
    l_from = move.parameter("l_from")
    l_to = move.parameter("l_to")
    move.add_condition(StartTiming(), Equals(loc, l_from))
    move.add_condition(overall, connected(l_from, l_to))
    move.add_effect(EndTiming(), loc, l_to)
    move.set_fixed_duration(10)
    htn_temporal.add_action(move)
