...
class TestPDDLPlanner(TestCase):
    ...
    def test_robot_loader_adv(self):
        ...
        with OneshotPlanner(name='enhsp') as planner:
            ...
            self.assertIn(f'\nRunning WA-STAR\nh(n = s_0)=3.0\n', planner_output)
            self.assertIn(f'f(n) = 3.0 (Expanded Nodes: 0, Evaluated States: 0, Time: ', planner_output)
            self.assertIn(f'f(n) = 4.0 (Expanded Nodes: 2, Evaluated States: 3, Time: ', planner_output)
            self.assertIn(f'f(n) = 5.0 (Expanded Nodes: 5, Evaluated States: 6, Time: ', planner_output)
