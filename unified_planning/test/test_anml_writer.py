...
class TestPythonWriter(TestCase):
    ...
    def test_timed_connected_locations(self):
        ...
        expected_result = [
            ...
            '[ start ] is_connected(l3, l4) := true;',
            '[ start ] is_connected(l4, l5) := true;',
            ...
            '[ start ] is_connected(l3, l1) := false;',
            '[ start ] is_connected(l4, l1) := false;',
            '[ start ] is_connected(l5, l1) := false;',
            '[ start ] is_connected(l2, l2) := false;',
            '[ start ] is_connected(l3, l2) := false;',
            '[ start ] is_connected(l4, l2) := false;',
            '[ start ] is_connected(l5, l2) := false;',
            '[ start ] is_connected(l1, l3) := false;',
            '[ start ] is_connected(l3, l3) := false;',
            '[ start ] is_connected(l4, l3) := false;',
            '[ start ] is_connected(l5, l3) := false;',
            '[ start ] is_connected(l1, l4) := false;',
            '[ start ] is_connected(l2, l4) := false;',
            '[ start ] is_connected(l4, l4) := false;',
            '[ start ] is_connected(l5, l4) := false;',
            '[ start ] is_connected(l1, l5) := false;',
            '[ start ] is_connected(l2, l5) := false;',
            '[ start ] is_connected(l3, l5) := false;',
            '[ start ] is_connected(l5, l5) := false;',
            '[ end ] is_at(l5);\n']
        self.assertEqual(anml_problem, '\n'.join(expected_result))
    ...
    def test_robot(self):
        ...
        expected_result = [
            'type Location;',
            'type boolean;',
            'type rational_0 < rational [0, 100];',
            'fluent boolean robot_at;',
            'fluent rational_0 battery_charge;',
            'action move(Location l_from, Location l_to) {',
