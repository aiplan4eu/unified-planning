...
class QuantifierSimplifier(Simplifier):
    ...
    def __init__(self, env: 'unified_planning.environment.Environment', problem: 'up.model.Problem'):
        DagWalker.__init__(self, True)
        self._env = env
        self.manager = env.expression_manager
        self._problem = problem
        self._assignments: Optional[Dict['up.model.Expression', 'up.model.Expression']] = None
        self._variable_assignments: Optional[Dict['up.model.Expression', 'up.model.Expression']] = None

    def qsimplify(self, expression: 'up.model.FNode', assignments: Dict['up.model.Expression', 'up.model.Expression'], variable_assignments: Dict['up.model.Expression', 'up.model.Expression']):
    ...
    def _push_with_children_to_stack(self, expression: 'up.model.FNode', **kwargs):
    ...
    def _compute_node_result(self, expression: 'up.model.FNode', **kwargs):
    ...
    def walk_exists(self, expression: 'up.model.FNode', args: List['up.model.FNode']) -> 'up.model.FNode':
        assert self._problem is not None
        assert len(args) == 1
        ...
        possible_objects: List[List['up.model.Object']] = [list(self._problem.objects(t)) for t in type_list]
        ...
        for o in product(*possible_objects):
            subs: Dict['up.model.Expression', 'up.model.Expression'] = dict(zip(vars, list(o)))
    ...
    def walk_forall(self, expression: 'up.model.FNode', args: List['up.model.FNode']) -> 'up.model.FNode':
        assert self._problem is not None
        assert len(args) == 1
        if args[0].is_bool_constant():
            if args[0].bool_constant_value():
    ...
    def walk_fluent_exp(self, expression: 'up.model.FNode', args: List['up.model.FNode']) -> 'up.model.FNode':
        ...
        if res is not None:
            ...
            assert type(res) is up.model.FNode
            return res
        else:
            raise UPProblemDefinitionError(f"Value of Fluent {str(expression)} not found in {str(self._assignments)}")

    def walk_variable_exp(self, expression: 'up.model.FNode', args: List['up.model.FNode']) -> 'up.model.FNode':
        assert self._variable_assignments is not None
